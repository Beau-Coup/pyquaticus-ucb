...
if __name__ == '__main__':
    ...
    reward_config = {'agent_0':rew.retrieve, 'agent_2':rew.zero} # Example Reward Config
    #Competitors: reward_config should be updated to reflect how you want to reward your learning agent

    args = parser.parse_args()
    logging.basicConfig(level=logging.ERROR)

    save_path = f"./ray_test/1v1/{datetime.now().strftime('%Y-%m-%d_%H:%M')}"
    ...
    def policy_mapping_fn(agent_id, episode, *args, **kwargs): # worker, **kwargs):
        if agent_id == 'agent_0':
    ...
    for i in range(8001):
        ...
        # print("Evaluation: ")
        # eval_info = algo.evaluate()
        # # eval_info: env_runners: {policy_reward_min: {agent-0-policy: number, ...}, policy_reward_max: {..}, policy_reward_mean: {..}}
        # # pprint.pp(eval_info)
        # print("Evaluation done")
        if np.mod(i, 10) == 0:
            print("Saving Checkpoint: ", i)
            chkpt_file = algo.save(f'{save_path}/iter_'+str(i)+'/')
